...
=======
    ...
    float longBlockHoldPenaltyPerSecond = -0.0025f;
    float blockHoldDecayPerSecond = 1.75f;

    [Header("Block Spam / Blocked Advance Hygiene")]
    float blockStartPenalty = -0.00012f;
    float aimlessBlockPenaltyPerSecond = -0.0012f;
    float blockedAdvancePenaltyPerSecond = -0.0015f;
    float blockInsteadOfParryPenalty = -0.00055f;
    float blockDisciplineRangeX = 1.55f;
    float blockDisciplineRangeY = 0.85f;
>>>>>>> Stashed changes
...
=======
    ...
    float defensivePassivityMultiplier = 1.7f;
    ...
    float bodyPushPenaltyPerSecond = -0.04f;
    float bodyPushBlockMultiplier = 2.2f;
...
=======
        ...
        BlockDisciplineRewards(moveX, blockHold);
...
=======
    ...
    void BlockDisciplineRewards(int moveX, int blockHold)
    {
        if (self == null || opp == null)
        {
            return;
        ...
        int towardOpponent = dx >= 0f ? 1 : -1;
        bool movingToward = moveX != 0 && moveX == towardOpponent;
        bool closeForDiscipline =
            absDx <= blockDisciplineRangeX &&
            absDy <= blockDisciplineRangeY;

        bool blockStarted = blockHold == 1 && lastBlockAction == 0;
        ...
        bool realPressure = HasRealActivePressure();

        if (blockStarted)
        {
            AddReward(blockStartPenalty);
            rewardDebugger?.LogBlockStartPenalty(blockStartPenalty);

            if (parryOpportunity)
            {
                AddReward(blockInsteadOfParryPenalty);
                rewardDebugger?.LogBlockInsteadParryPenalty(blockInsteadOfParryPenalty);
            }
        }

        if (blockHold == 1 && closeForDiscipline && !immediateThreat && !obviousPunish && !realPressure)
        {
            float penalty = aimlessBlockPenaltyPerSecond * dt;
            AddReward(penalty);
            rewardDebugger?.LogAimlessBlockPenalty(penalty);
        }

        if (blockHold == 1 && closeForDiscipline && movingToward && !realPressure)
        {
            float penalty = blockedAdvancePenaltyPerSecond * dt;
            AddReward(penalty);
            rewardDebugger?.LogBlockedAdvancePenalty(penalty);
        }

        lastBlockAction = blockHold;
    }
